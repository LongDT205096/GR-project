...
<div className="max-w-sm p-3 mx-auto bg-neutral-700 rounded-lg shadow dark:bg-gray-800">
    <div className="flex flex-col items-center pb-10">
        ...
        <img className="w-24 h-24 mb-3 rounded-full shadow-lg" src={user.photoURL ? user.photoURL : './assets/img/profile.jpg'} alt={user.displayName} />
        <h5 className="mb-1 text-xl font-medium text-white dark:text-white">{user.displayName}</h5>
        <span className="text-sm text-white dark:text-gray-200">{user.email}</span>
        <div className="flex mt-4 space-x-3 md:mt-6">
            <button onClick={()=>auth.signOut()} className="inline-flex items-center px-4 py-2 text-sm font-medium text-center text-gray-900 bg-white border border-gray-300 rounaded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-700 dark:focus:ring-gray-700">Logout</button>
        </div>
    </div>
</div>
</div>
