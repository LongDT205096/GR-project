...
def main():
    with open("data/actor_id.json", "r") as f:
        data = json.load(f)
        f.close()
    with open("data/movie_crew_list.json", "r") as f:
        movies = json.load(f)
        f.close()
    movie_keys = list(movies.keys())
    for title in movie_keys:
        for actor in movies[title]:
            actor["db_id"] = data[actor["actor_name"]]
    with open("data/movie_crew_list_2.json", "w") as f:
